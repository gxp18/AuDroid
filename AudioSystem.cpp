...
#include <stdlib.h> //gxp18
#include <binder/IPCThreadState.h> //gxp18
...
int callerPIDGXP18 = 0; // used to synch AudioSystem.cpp and AudioFlinger.cpp on callerPID for access to mike 
                                 ...
                                 audio_session_t session)
{
    ...
    int ret = system("/system/bin/am broadcast -a android.intent.action.MIKE_ACCESSED --user all");  // gxp18 - Need to set SELinux in Permissive mode

    pid_t pid = IPCThreadState::self()->getCallingPid();
    int uid = IPCThreadState::self()->getCallingUid();
    
    __android_log_print(ANDROID_LOG_DEBUG,"gxp18 - Mutex", "InSTART Session %d CallerPID %d UID %d ID %d", session, pid, uid, callerPIDGXP18);	
...
status_t AudioSystem::stopInput(audio_io_handle_t input,
                                audio_session_t session)   // gxp18 - audio_io_handle_t is an int - audio_session_t is a pair of int - not useful for tracking app
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();

    int ret = system("/system/bin/am broadcast -a android.intent.action.MIKE_RELEASED --user all"); // gxp18 - Need to set SELinux in Permissive mode

    pid_t pid = IPCThreadState::self()->getCallingPid();
    int uid = IPCThreadState::self()->getCallingUid();
   
    callerPIDGXP18 = pid;
    __android_log_print(ANDROID_LOG_DEBUG,"gxp18 - Mutex", "InSTOP Session %d CallerPID %d UID %d ID %d", session, pid, uid, callerPIDGXP18);	
