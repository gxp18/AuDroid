...
class ActivityManagerProxy implements IActivityManager
{
    ...
    {
	Log.d("gxp18", "ActivityManagerNative.java broadcasting intent UID " + userId + " Intent " + intent.toString()); //gxp18
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeStrongBinder(caller != null ? caller.asBinder() : null);
        ...
        if ((userId == -1) && ((intent.toString()).equals("Intent { act=android.intent.action.MIKE_ACCESSED }"))) data.writeInt(0);// gxp18 - Hack 
	else if ((userId == -1) && ((intent.toString()).equals("Intent { act=android.intent.action.MIKE_RELEASED }"))) data.writeInt(0); // gxp18 - Hack
        else data.writeInt(userId);
        mRemote.transact(BROADCAST_INTENT_TRANSACTION, data, reply, 0); // mRemote is an instance of IBinder
        Log.d("gxp18", "ActivityManagerNative.java transact sent"); //gxp18
        reply.readException();
        int res = reply.readInt();
	Log.d("gxp18", "ActivityManagerNative.java reply received. ret = " + res); //gxp18
        reply.recycle();
        data.recycle();
        return res;
    }
